...
(* Todo: make a more general version of [Neural_network.create_exn], and incoporate [Autodiff.Float.compose] somehow - use an (int * int) Set.t list *)
...
let main ~ignored:_ =
  ...
  (* let nn = Neural_network.create_exn [[0,0;0,1;0,2;1,0;1,1;1,2];[0,0;1,0;2,0]] in *)
  ...
  let out = nn.Neural_network.output (List.map [3.; 4.] ~f:Autodiff.Float.c) in
  printf "%s\n" (Sexp.to_string_hum (List.sexp_of_t Neural_network.Parameter.sexp_of_t nn.Neural_network.parameters));
  (* let res = Autodiff.Float.eval' (List.hd_exn out) [-0.5;-0.1;0.2;-0.18;0.32;0.21;-0.25;0.1;0.02;0.22;-0.18;0.23;-0.3] in *)
  (* let res = Autodiff.Float.eval' (List.hd_exn out) [0.22;-0.18;0.23;-0.3;-0.5;-0.1;0.2;-0.18;0.32;0.21;-0.25;0.1;0.02] in *)
